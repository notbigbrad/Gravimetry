...
def prop(time, subtended_angle, constants, constants_std, p0, n=1e3, processors=None, **kwargs):
    """
    Runs a Monte Carlo simulation to estimate the uncertainty in a system modeled by an ODE.

    Parameters:
    - time: Time values for simulation.
    - subtended_angle: Observed data to fit.
    - constants: Mean values of system parameters.
    - constants_std: Standard deviation for each parameter.
    - p0: Initial guess for curve fitting.
    - n: Number of Monte Carlo samples (default: 1000).
    - processors: Number of CPU cores to use (default: system CPU count - 3).

    Returns:
    - results: Estimated parameters from fitting.
    - errors: Uncertainties in estimated parameters.
    - summary: [Mean, Standard Error] of results.
    """

    if processors is None:
        processors = max(1, multiprocessing.cpu_count() - 3)  # Prevent overloading CPU

    def ODE_wrapper(i, arrays, x, y):
        optimal, covariance = optimize.curve_fit(
            lambda t, *fittedParams: ode_callable_über_wrapper(
                t, *fittedParams, m_given=arrays[i][1], r_o_given=arrays[i][2], I_given=arrays[i][0]
            ), x, y, p0=p0, maxfev=9999999
        )
    ...
    noise = list(map(list, zip(*noise)))  # Convert NumPy array to list of lists

    results, errors = zip(*Parallel(n_jobs=processors, backend="loky")(
